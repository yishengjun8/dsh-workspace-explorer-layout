...
| ![四栏布局总览](image/image-1.png) | ![文件视图与编辑器](image/image-2.png) |
|---|---|
...
| 能力 | 说明 |
|---|---|
| 📁 **工作区文件树** | 会话归属 Workspace 时自动出现，目录优先、逐级展开，刷新后**展开状态与滚动位置按会话恢复** |
...
- 侧栏顶部提供「会话列表 / 文件浏览」视图切换，会话标题右键可重命名当前会话。
- 可拖拽调宽会话栏、文件树栏与编辑栏（打开时左侧最多 80%），布局参数以 `localStorage` 全局持久化。
...
- 面板头提供「自动换行」与「从磁盘重新读取」（刷新）开关；刷新遇到未保存草稿会拒绝并提示。
...
- 草稿缓存保留在页面内存、不写入 `localStorage`；切换文件不会静默丢弃未保存内容。
- 标签条支持滚轮横向滚动，打开新标签自动滚动到可见。

### 编辑与保存

- 显式进入编辑模式，提供“保存”“取消”与 `Ctrl/Cmd+S`，不会自动保存。
...
- 聊天中正在输出的思考内容（Think 条）默认自动展开、结束后按可调延迟自动收起（0–10 秒，分度 0.1 秒，默认 3 秒，期间手动操作可取消），用户手动操作优先；可在资源管理器设置页关闭该行为并调整延迟。
- `/init` 命令（类似 Claude Code）：在当前会话所属工作区的根目录生成或更新 `AGENTS.md`，已有文件时弹层让你选择「更新」或「取消」，由当前 Agent 分析工作区后生成。
- 可将外部文件拖入预览面板直接以只读标签预览（会话内有效，不写入工作区）。

### 外观与设置

- 使用 Harness 主题语义变量，支持亮色、暗色与系统主题。
- 资源管理器设置页：文件树行高、搜索结果显示方式、文件图标徽标配色、每类文件高亮预设、对话文字大小、Think 条自动展开与延迟。
...
脚本优先使用 PATH 中的 `dsh`；当前目录属于 Harness checkout 且 PATH 无 `dsh` 时自动使用 `pnpm --dir <harness-root> dsh`，也可用 `DSH_BIN` 指定可执行文件。安装完成后**停止并重启原有 Web 进程**，然后刷新 `http://127.0.0.1:3080`；脚本不会启动第二个服务器。
