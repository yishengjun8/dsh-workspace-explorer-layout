...
- 搜索内容时结果按文件分组：点击文件头折叠/展开该文件的匹配条目，点击匹配条目打开文件并跳到对应行；可在资源管理器设置页选择搜索结果的默认展开/折叠方式。
- 界面语言跟随 Harness「设置 → 通用设置 → 语言」（中文 / English）即时切换；英文界面下编辑器上下文提示与插件 API 错误信息同样本地化，无需重启或刷新。
...
- 会话栏、文件树栏与编辑栏均可拖动调宽（打开时左侧最多 80%），布局参数以 `localStorage` 全局持久化。
- 侧边栏底部另提供“手机模式”开关（与 dsh-mobile-preview 同思路）：开启后整栏布局切换为居中的手机竖屏列，侧栏变为由左上角鲸鱼开合的悬浮抽屉（会话列表与文件树仍在其中），会话头部鲸鱼右侧出现“文件内容浏览”按钮，点击后文件浏览（预览标签 + 编辑器）铺满手机列、会话头部保持可操作；再次点击或退出手机模式即恢复桌面布局。手机模式为瞬态状态，刷新后回到桌面布局。
