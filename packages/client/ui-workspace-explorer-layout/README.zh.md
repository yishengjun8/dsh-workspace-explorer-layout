...
- 文件树开头的类型徽标按文件类型颜色分组着色（目录、TypeScript、JavaScript、JSON、标记、样式、Markdown、日志、Python、C#、Shell、配置、C 系、其他、受阻）。浏览器设置页新增「资源管理器设置」标签页，把资源管理器的全部偏好集中在一起并按三大类分组：「文件浏览设置」（文件树行高、搜索结果默认展开/折叠、按分组自定义的图标颜色方案，即时预览）、「内容浏览设置」（按文件类型选择的代码高亮预设：默认、经典、暖色、冷色、单色，外加每种代码语言对应的 VS Code 配色与 Visual Studio 2022 配色）、「对话框设置」（对话文字大小、思考过程自动展开开关——默认开启，聊天中正在输出的思考内容自动展开、结束后按可调延迟自动收起，0–10 秒、分度 0.1 秒、默认 3 秒，期间手动操作可取消，用户手动操作优先——以及思考收起延迟）。每个语言分组默认采用各自的 VS Code 预设——XML、Python、JSON、TypeScript、JavaScript、CSS、Markdown、Shell、配置与 C/C++——而 C# 默认采用 Visual Studio 2022 预设——并支持单独恢复行高或字号、一键恢复全部颜色或全部预设。
- Browser 入口还在 `ui-commands` 的 `/` 斜杠菜单中注册 `init` 命令（类似 Claude Code 的 `/init`）：解析当前会话所属工作区，弹出确认项并显示目标根目录，向当前 Agent 发送「分析工作区并生成/更新根目录 `AGENTS.md`」的指令（已有文件时保留有价值内容合并更新，不盲目覆盖）。仅直接会话可用；无 `commandUi` 服务时自动不注册。
